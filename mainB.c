...
void rtupdate(struct distance_table *dt, struct rtpkt recv_pkt)
{
    ...
    for(int i=0;i<num_nodes;i++){ 
      ...
      for(int j=0;j<num_nodes;j++){
        ...
        if(link_costs[dest][j]!=-1 && myNodeDts[j][i]!=-1 && dest != j){
          ...
          } else if(min_cur == link_costs[dest][j] + myNodeDts[j][i]) {
...
int main(int argc, char *argv[])
{
  ...
  if (file2 == NULL)
  {
    ...
    while(cur_simulation<=k_max) {
      ...
      printf("k=%d:\n",cur_simulation);
      for(int i=0;i<num_rows_in_traffic;i++){
        int src = 0, dest = 0, hop = 0, hop_set = 0;
        for(int j=0;j<3;j++){
          if(j == 0){
        ...
        for(int k=0;k< f_table[src].dest[dest].hop_index ; k++){
          printf("%d>",f_table[src].dest[dest].hop[k]);
          hop = f_table[src].dest[dest].hop[k];
          hop_set = 1;
          fflush(stdout);
        }
        if(hop !=dest && hop_set ==1)
          printf("%d",dest);
        else 
          printf("%d",dest);
